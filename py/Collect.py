...
class Collection(Thread):
    ...
    def setup_scope(self):
        ...
        self.scope.chunk_size = int(self.config['settings'][2]+11)  # *1.59)
    ...
    def run(self):
        ...
        while run:
            ...
            # time.sleep(0.1)
            osc_data = self.scope.read_bytes(int(self.config['record_length'] + 11))  # int(self.config['frame_count']*self.config['record_length']))
            # self.scope.write('CURVE?')
